...
const Footer = () => {
    ...
    const linkSections = [
        ...
        },
        {
            title: "CONTACT",
            links: [
                { text: "+91 98765 43210", path: 'tel:+919876543210', icon: PhoneIcon },
                { text: "support@anantin.com", path: 'mailto:support@anantin.com', icon: MailIcon },
                { text: "Ring Road, Surat, Gujarat, India", path: '/', icon: MapPinIcon }
            ]
        }
    ];
    ...
    return (
        <footer className="mx-6 bg-[#2C241B] rounded-t-2xl">
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-[#3F332A] text-[#EAE0D5]">
                    <div>
                        <Link href="/" className="relative flex items-center gap-3">
                            <div className="relative w-[40px] h-[40px]">
                                <Image src="/anantin%20logo.png" alt="Anantin Logo" fill className="object-contain opacity-90" />
                            </div>
                            <div className="relative text-3xl font-semibold text-[#FDFBF7]">
                                <span className="text-[#a69d8b]">anan</span>tin<span className="text-[#D4B26F] text-4xl leading-0">.</span>
                                <p className="absolute text-[8px] font-bold uppercase -top-1 -right-[38px] px-2 py-0.5 rounded-full flex items-center gap-2 text-[#2c3e50] bg-[#D4B26F]">
                                    LUXE
                                </p>
                            </div>
                        </Link>
                    ...
                    <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-5 text-sm">
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-semibold text-[#FDFBF7] md:mb-5 mb-3">{section.title}</h3>
                                <ul className="space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2 text-[#EAE0D5]/90 hover:text-[#FDFBF7]">
                                            {link.icon && <link.icon />}
                ...
                <p className="py-4 text-sm text-[#8C8A85]">
                    Copyright 2025 © anantin All Right Reserved.
                </p>
            </div>
        </footer>
    );
